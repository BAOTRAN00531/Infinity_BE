...
VALUES (N'Khoá học mẫu', 1, 1);  -- 1 là id của admin user và language
...
VALUES (
    1,               -- course_id
    1,               -- lesson_id
    N'Câu hỏi mẫu: Thủ đô của Việt Nam là gì?', -- question_text
    1,               -- question_type_id (1 = multiple_choice_single)
    NULL, NULL, NULL,
    1,               -- created_by
    'easy',
    10
...
INSERT INTO dbo.Languages (code, name) VALUES
('vi', 'Vietnamese'),
('en', 'English'),
('zh', 'Chinese'),
('ja', 'Japanese'),
...
INSERT INTO LanguageTemplates (code, name) VALUES
('vi', 'Vietnamese'),
('en', 'English'),
('jp', 'Japanese'),
('cn', 'Chinese');
SELECT * FROM LanguageTemplates;
SELECT COUNT(*) FROM LanguageTemplates;
...
VALUES
    ('admin', 'anyemail@example.com', N'Tên nào cũng được', 'admin' ,'$2a$12$gSBFLoRiPCU5vtJSVKx9qej0wqfhVvgsfpZRiB/yoMR93aBtzf4Bu', 1);

INSERT INTO dbo.Users 
    (username, email, full_name, role, password, is_active)
VALUES
    ('admin1', 'anyemail1@example.com', N'Tên nào cũng được', 'admin' ,'$2a$12$7qNKP7SVXS0.fddyBLqeKuyjU6IS/zv/DFY8nLnciMo/LAzZ9HqJi', 1);



	--UPDATE dbo.Users
--SET password = '$2a$12$gSBFLoRiPCU5vtJSVKx9qej0wqfhVvgsfpZRiB/yoMR93aBtzf4Bu'
--WHERE username = 'admin';



-- 3.2. Sample Question Types
...
-- 4.1. Trigger cập nhật phân cấp khi chèn/đổi Lessons
...
BEGIN
    ...
    SELECT TOP(1)
        @admin_id = COALESCE(i.updated_by, i.created_by)
    FROM inserted i;

    -- 1) Cascade cập nhật Modules
    UPDATE m
    SET 
    ...
    INSERT INTO dbo.Admin_Audit_Log (
        admin_id, action, target_table, target_id, details, action_time
    )
    SELECT
        @admin_id,
